...
import { updateMissionProgress } from '$lib/server/missions.js';
...
export async function POST({ request }) {
	try {
		...
		// 미션 진행 업데이트
		const result = updateMissionProgress(userId, body.gameType, body.gameResult);
		...
		console.error('Mission update API POST error:', error);
		return json(
			{
				success: false,
				error: '서버 오류가 발생했습니다.'
